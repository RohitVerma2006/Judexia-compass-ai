...
import { Scale, Clock, CheckCircle, User, Star, MapPin, CalendarIcon, CreditCard, IndianRupee, Loader2 } from 'lucide-react';
...
export default function Consultation() {
  ...
  useEffect(() => {
    const fetchLawyers = async () => {
      ...
      const { data } = await supabase
        .from('profiles')
        .select('user_id, full_name, specialization, experience, rating, location, consultation_fee, avatar_url')
        .not('specialization', 'is', null);

      // Filter to only lawyers (those with user_roles = lawyer)
      if (data && data.length > 0) {
        ...
        const lawyerIds = new Set((roles || []).map((r: any) => r.user_id));
        setLawyers(data.filter((d: any) => lawyerIds.has(d.user_id)));
      } else {
        setLawyers([]);
      }
  ...
  const filteredLawyers = category
    ? lawyers.filter(l => l.specialization === category)
    : lawyers;

  const handleBookClick = (lawyer: LawyerProfile) => {
  ...
  const handlePayNow = async () => {
    ...
    if (user && selectedLawyer && date) {
      await supabase.from('consultations').insert({
        ...
        category: selectedLawyer.specialization || 'General',
        fee: selectedLawyer.consultation_fee || 0,
        status: 'pending',
        payment_status: 'paid',
      });
      ...
      await supabase.from('notifications').insert({
        ...
        message: `New consultation request from ${user.fullName} for ${selectedLawyer.specialization || 'Legal'} on ${format(date, 'PPP')} at ${time}.`,
      });
    }
    ...
    return (
      <div className="animate-fade-in flex items-center justify-center min-h-[60vh]">
        <div className="glass-card p-8 text-center max-w-md">
          ...
          <p className="text-muted-foreground mb-1">Booking Request Sent to {selectedLawyer?.full_name}</p>
    ...
    <div className="animate-fade-in space-y-6">
      ...
      {loadingLawyers ? (
        ...
        <div className="text-center py-12 text-muted-foreground">
          ...
          <p>No lawyers available{category ? ` for ${category}` : ''} yet.</p>
          <p className="text-xs mt-1">Lawyers will appear here after they register and set up their profile.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredLawyers.map(lawyer => (
            <div key={lawyer.user_id} className="glass-card p-5 hover:border-electric/30 transition-all duration-300">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-14 h-14 rounded-full bg-gold/10 flex items-center justify-center shrink-0">
                  <User className="w-7 h-7 text-gold" />
                </div>
                <div>
                  <h4 className="font-serif font-semibold">{lawyer.full_name}</h4>
                  <p className="text-xs text-muted-foreground">{lawyer.specialization || 'General Practice'}</p>
                </div>
              </div>
              <div className="space-y-2 text-sm mb-4">
                <div className="flex items-center gap-2">
                  <Star className="w-3.5 h-3.5 text-gold fill-gold" />
                  <span className="text-gold font-medium">{lawyer.rating || '4.5'}</span>
                  <span className="text-muted-foreground">•</span>
                  <span className="text-muted-foreground">{lawyer.experience || 'Experienced'}</span>
                </div>
                {lawyer.location && (
                  <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-electric font-semibold">₹{lawyer.consultation_fee || 500}</span>
          ...
          {selectedLawyer && (
            <div className="space-y-4">
              <div className="flex items-center gap-3 p-3 rounded-lg bg-secondary/30">
                ...
                <div>
                  <p className="font-semibold text-sm">{selectedLawyer.full_name}</p>
                  <p className="text-xs text-muted-foreground">{selectedLawyer.specialization} • ₹{selectedLawyer.consultation_fee || 500}</p>
                </div>
              </div>
              ...
              <div className="flex justify-between items-center p-3 rounded-lg bg-gold/5 border border-gold/20">
                ...
                <span className="text-lg font-bold text-gold">₹{selectedLawyer.consultation_fee || 500}</span>
              </div>
          ...
          {processing ? (
            ...
            <div className="space-y-4">
              ...
              <div className="p-3 rounded-lg bg-gold/5 border border-gold/20 flex justify-between items-center">
                ...
                <span className="text-xl font-bold text-gold">₹{selectedLawyer?.consultation_fee || 500}</span>
              </div>
